...
def poster_grid(cards, cols=6, key_prefix="grid"):
    ...
    for r in range(rows):
        ...
        for c in range(cols):
            ...
            with colset[c]:
                # Card wrapper so styles apply consistently
                ...
                # Use an HTML link to navigate (avoids Streamlit button key collisions
                # inside dynamic grids). The link sets query params so the app can
                # pick them up and show details.
                if tmdb_id:
                    link = f"?view=details&id={int(tmdb_id)}"
                    st.markdown(
                        f"<a href='{link}' style='display:inline-block;padding:8px 10px;background:#0f62fe;color:#fff;border-radius:8px;text-decoration:none;margin-top:6px'>Open</a>",
                        unsafe_allow_html=True,
                    )
